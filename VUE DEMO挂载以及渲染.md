...
在引入 `Vue`的过程中  注入到 `Vue`原型上。 可以参照  Vue 源码阅读笔记的引入路径
...
它做了这些事情：定义`render`方法、触发生命周期钩子函数 `beforeMount`、生成 `vnode`.、生成真实dom节点、 触发`beforeUpdate` || `nounted`钩子函数 .
